    ...
    <div className="min-h-screen bg-slate-950 p-6 relative overflow-hidden font-sans">
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="glass-card">
            <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-1">Live PNL</p>
            <p className={`text-xl font-bold ${getPnlClass(data.live_pnl)}`}>{data.live_pnl}</p>
          </div>
          <div className="glass-card">
            <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-1">Daily PNL</p>
            <p className={`text-xl font-bold ${getPnlClass(data.daily_pnl)}`}>{data.daily_pnl}</p>
          </div>
          <div className="glass-card">
            <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-1">Win Rate</p>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-700/50 text-slate-400 text-sm">
                      ...
                      <th className="py-3 px-4 font-semibold">Targets</th>
                  ...
                  <tbody>
                    {data.active_positions && data.active_positions.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan="6" className="py-8 text-center text-slate-500">No active positions</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {data.closed_trades && data.closed_trades.length > 0 && data.closed_trades[0][0] !== '-' ? (
                      ...
                      <tr>
                        <td colSpan="6" className="py-8 text-center text-slate-500">No recent trades</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          <div className="glass-panel p-6 flex flex-col h-[600px] lg:h-auto">
            ...
            <div className="flex-1 bg-slate-950/80 rounded-lg border border-slate-800 p-4 font-mono text-sm overflow-y-auto terminal-scroll shadow-inner">
              {data.logs ? (
                ...
                <div className="text-slate-600 italic">Waiting for terminal output...</div>
              )}
              <div ref={logsEndRef} />
            </div>
          </div>
